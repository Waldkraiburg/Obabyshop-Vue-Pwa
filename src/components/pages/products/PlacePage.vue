<template>
  <div>
    <section class="categories">
      <h1 class="categories__title">{{ city }}</h1>
      <div class="categories__search-box">
        ...
        <input
          ...
          type="search"
          id="search"
          :placeholder="'Rechercher dans la ville de ' + city"
          autocomplete="off"
        />
      </div>
      <span
...
export default {
  ...
  computed: {
    filteredList() {
      return this.cityProducts.filter((product) => {
  ...
  mounted: function () {
    ...
    ApiClient.get(
      "/wp/v2/products/?meta_key=localite_produit&meta_value=" + city
    ).then((response) => {
      this.cityProducts = response.data;
    });
  },
  methods: {
    /* eslint-disable no-unused-vars */
    sortLowest() {
      let sortedProducts = this.cityProducts.sort(
